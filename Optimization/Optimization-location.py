...
def preprocess_rbm(folder, outlet_id):
    with open('/glade/u/home/dblaskey/RBM/Optimization/prepare_rbm_opt_%s_%s.cfg'%(folder,outlet_id), 'w') as f:
        ...
        f.write("input_id: %s\n"%outlet_id)
        f.write("optimization_param: /glade/u/home/dblaskey/RBM/Optimization/Opt_runs_%s_%s.csv\n"%(iteration,local))
...
pre_lines = ["[INPUT]", 
"output_energy_nc: /glade/scratch/tcraig/archive/NNA.4km.hERA5.1989.003.pp/lnd/hist/NNA.4km.hERA5.1989.003.clm2.hrbm2.", "","[PARAM]", "start_date: 20130101", "end_date: 20171231", "seg_minflow : 4", "sd_time_step: 1", "", "[RBM_OPTIONS]", "min_flow : 5", "", "[OPT]"]
...
for iteration in range(16,21):
    ...
    for local in np.unique(df_sites.Location.values):
        print(local)
        path_opt = '/glade/u/home/dblaskey/RBM/Optimization/Opt_runs_%s_%s.csv'%(iteration+1, local)
        
        if os.path.isfile(path_opt) == False:
            print("Create file")
            outlets = df_sites[df_sites['Location'] == local].outlet_comid.values

            df_LHS_0 = pd.read_csv('/glade/u/home/dblaskey/RBM/Optimization/LHS_results_0_%s.csv'%local)

            d = preprocessing.normalize(df_LHS_0.drop(["Name", "temp_rmse"], axis = 1),axis=0,return_norm=True)
            normalization_scalar = d[1]

            psets_df = pd.read_csv('/glade/u/home/dblaskey/RBM/Optimization/Opt_runs_%s_%s.csv'%(iteration,local))
            ...
            df2.to_csv('/glade/u/home/dblaskey/RBM/Optimization/LHS_results_raw_loc_%s_%s.csv'%(iteration,local), index=False)

            df_LHS = df_temp.groupby('Name').mean("temp_rmse").reset_index().drop(columns=['COMID', 'temp_pbias', 'Outlet'])
            df_LHS = df_LHS.merge(psets_df)

            df_LHS_pre = pd.read_csv('/glade/u/home/dblaskey/RBM/Optimization/LHS_results_%s_%s.csv'%(iteration - 1, local))
            df_LHS = pd.concat([df_LHS_pre, df_LHS]).reset_index(drop=True)

            df_LHS.to_csv('/glade/u/home/dblaskey/RBM/Optimization/LHS_results_%s_%s.csv'%(iteration, local), index=False)
            ...
            for id_ in range(N_resample):
                test_id = '%s_%s_%s_%s'%(location, local, "%i"%(iteration+1), "%04i"%(id_))
            ...
            psets_df_new.to_csv('/glade/u/home/dblaskey/RBM/Optimization/Opt_runs_%s_%s.csv'%(iteration+1, local))
