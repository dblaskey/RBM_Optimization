...
cfg = read_config(sys.argv[1])  # Read config file
...
Ordered_reaches_final['seg_num'] = round_up_to_even(Ordered_reaches_final['length']*0.000621371/24) + min_seg
...
for i in range(len(data)):
    ...
    if os.path.exists(file_path) == True:
        print("Skip ", input_id, " in ", folder)            
    else:                                    
        print("Creating ", input_id, " in ", folder) 
...
for year in years:
    ...
    for i in range(len(data)):
        ...
        if os.path.exists(file_path) == True:
            ...
            da_velocity.values[da_velocity.values<cfg['RBM_OPTIONS']['min_velocity']] = cfg['RBM_OPTIONS']['min_velocity'] 

            #=== Recalculate width and depth ===#
            da_depth = da_flow / da_velocity / da_width  # flow depth [ft]
